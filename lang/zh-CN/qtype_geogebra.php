...
$string['answervar_help'] = '对自动评分: 如果学生解决了问题(部分地), 那么 GeoGebra 中的布尔运算对象名称为真. 合计所有布尔变量的所有分数. 如果任何组合都超过了 100%, 那么问题就是正确的, 但是至少应该有一个组合的总分恰恰达到 100%. 为手动评分留下空白.';
...
$string['ggbturl_help'] = '你或者可以使用 GeoGebraTube 中的分享按钮复制粘贴链接, 或者使用 GeoGebraTube 库. Java程序和参数保存在数据库中, 除非有要求, Java程序将会从 GeoGebraTube 中重新载入. 只需提供 ID 或 Java程序的共享密钥也能得到支持.';
...
$string['isexercise_help'] = '该Java程序含有用户定义的可用于自动检查练习的工具.\n注意: 下面所有答案在此案例中不适用!';
...
$string['stepzero'] = '变量 {$a} 的增量为0; 你或者还没有指定滑动条的增量, 或者该元素根本就不是一个滑动条. 你可能不得不在 GeoGebra 文件中纠正它.';
